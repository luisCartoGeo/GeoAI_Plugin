...
class dialog_segmentar_i(DialogUi, DialogType):
    ...
    def __init__(self,iface,listaRaster):
        ...
        #variable para imagen sin sistema de coordenadas
        self.nogeo=False
...
#        if self.nareas.value()<=10:
    ...
    #metodo devuelve la imagen como arreglo apto para el modelo
...
#            print(nbandas)
        ...
        #print('geo origina ',geotransform[0],geotransform[3])
        ...
        #----crear geotransform y filas y columnas
            ...
            if self.nogeo:
                p11=p1[1]*-1
                p12=p2[1]*-1
                ars=arr[p11:p12,p1[0]:p2[0]]
            filas=ars.shape[0]
            columnas=ars.shape[1]
            preal=pixel_coord(geotransform,p1[0],p1[1])
            #print('p real ',preal,' filas y columnas ',filas, columnas)
            ...
            for n in l:
                ...
                if self.cortarImagen==True and not self.rectanguloCorte is None:
                    ...
                    if self.nogeo:
                        ...
                        py1=p1[1]*-1
                        py2=p2[1]*-1
                        ba=ba[py1:py2,p1[0]:p2[0]]
                        #print('forma',ba.shape)
                    else:
...
#            sam.eval()
        ...
        if self.defecto.isChecked():
            ...
            #carpeta temporal
            if len(lmask)==0 or lmask[0] is None:
                self.iface.messageBar().pushMessage('ERROR',\
            ...
            if len(listap)>0:
                ...
                ms.exec()
                return None
            resultado=crear_capa_salida(listcapas,epsg,self.iface,multi=False)
            if self.nogeo:
